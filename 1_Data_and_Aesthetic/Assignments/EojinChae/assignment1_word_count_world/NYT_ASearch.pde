...
int[] doASearchYears(String q, int pageNum, int startYear, int endYear) {
  int[] counts = new int[endYear - startYear];
  int i = startYear;
  while (i < endYear) {
    println(i);
    try {
      ASResult r = doASearch(q, pageNum, i + "0101", i + "1231");
      println(r.hits);
...
ASResult doASearch(String q, int pageNum, String beginDate, String endDate) {
  ASResult result = new ASResult();
  result.resultJSON = loadJSONObject(getASURL(q, pageNum, beginDate, endDate));
  result.parse();
  return(result);
}

String getASURL(String q, int pageNum, String beginDate, String endDate) {
//  return(baseURL + "q=" + java.net.URLEncoder.encode(q) + "&begin_date=" + beginDate + "&end_date=" + endDate + "&api-key=" + apiKey); // original line
  return(baseURL + "q=" + q + "&page=" + pageNum + "&begin_date=" + beginDate + "&end_date=" + endDate + "&api-key=" + apiKey);
}
...
class ASDoc {
  ...
  void parse() {
    ...
    word_count = int(docJSON.getString("word_count"));
    try {
